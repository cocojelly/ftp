...
int server = -1, client = -1, running = 0, pass_ok = 0, logged_in = 1;
pid_t forkpid = 1;
char buf[BUF_SIZE];
...
static MD5_CTX MDString(char *inString)
{
	int i, j;
	MD5_CTX mdContext;
	char modinString[20];
	...
	for (i = 0; ; i++)
		if ((*(inString + i)) < ' ') break;
	for (j = 0; j<i; j++)
		modinString[j] = *(inString + j);
	len = i;
	MD5Init(&mdContext);
	MD5Update(&mdContext, modinString, len);
	MD5Final(&mdContext);
	return mdContext;
}

/** PASS command */
void ftp_pass(char *cmd)
{
	int i;
	char *cmdarg;
	char okpass[32];
	MD5_CTX OK;
	FILE *p = fopen("okpass", "r");
	if (p == NULL) {
		return;
	}
	cmdarg = cmd;
	OK = MDString(cmdarg);
	sprintf(cmdarg, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", OK.digest[0], OK.digest[1], OK.digest[2], OK.digest[3], OK.digest[4], OK.digest[5], OK.digest[6], OK.digest[7], OK.digest[8], OK.digest[9], OK.digest[10], OK.digest[11], OK.digest[12], OK.digest[13], OK.digest[14], OK.digest[15]);
	while (1) {
		fgets(okpass, 33, p);
		sprintf(okpass, "%s", okpass);
		if (strcmp(okpass, cmdarg) == 0) {
			pass_ok = 1;
			break;
		}
		if (feof(p) != 0) break;
